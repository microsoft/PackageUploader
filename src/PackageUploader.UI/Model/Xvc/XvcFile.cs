...
class XvcFile
{
    ...
    /// The implementation lives in PackageUploader.ClientApi so the CLI and the UI share one source of truth.
    /// </summary>
    public static bool IsLikelyMsixvc2Package(string packagePath) =>
        PackageFormatDetector.IsLikelyMsixvc2Package(packagePath);
