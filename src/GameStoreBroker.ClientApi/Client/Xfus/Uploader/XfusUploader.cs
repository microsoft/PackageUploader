...
namespace GameStoreBroker.ClientApi.Client.Xfus.Uploader
{
    internal class XfusUploader : IXfusUploader
    {
        ...
        public XfusUploader(ILogger<XfusUploader> logger, UploadConfig uploadConfig)
        {
            _logger = logger;
        ...
        public async Task UploadFileToXfusAsync(string filePath, XfusUploadInfo xfusUploadInfo)
        {
            var uploadFile = new FileInfo(filePath);
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                httpClient.Timeout = TimeSpan.FromMilliseconds(_uploadConfig.HttpUploadTimeoutMs);

                var uploadProgress = await InitializeAssetAsync(httpClient, authToken, xfusUploadInfo.XfusTenant, assetGuid, properties).ConfigureAwait(false);
                ...
                while (uploadProgress.Status != UploadStatus.Completed)
                {
                    ...
                    {
                        await Task.Delay(uploadProgress.RequestDelay).ConfigureAwait(false);
                    }
                        ...
                        uploadProgress.PendingBlocks != null && uploadProgress.PendingBlocks.Any())
                    {
                        ...
                        bytesUploaded = await UploadBlocksAsync(
                            httpClient,
                            uploadProgress.PendingBlocks,
                            maximumBlockSize,
                            uploadFile,
                            assetGuid,
                            authToken,
                            xfusUploadInfo.XfusTenant,
                            bytesUploaded)
                    ...
                    try
                    {
                        uploadProgress = await ContinueAssetAsync(httpClient, authToken, xfusUploadInfo.XfusTenant, assetGuid).ConfigureAwait(false);
                    ...
                    catch (XfusServerException serverException)
                    {
                        ...
                        {
                            await Task.Delay(serverException.RetryAfter.TotalMilliseconds > 0 ? serverException.RetryAfter : new TimeSpan(_uploadConfig.HttpTimeoutMs)).ConfigureAwait(false);
                        }
                    }
                }
            }
        ...
        private async Task<UploadProgress> InitializeAssetAsync(HttpClient httpClient, string authToken, string tenant, Guid assetId, UploadProperties properties)
        {
            ...
            var uploadProgress = await response.Content.ReadFromJsonAsync<UploadProgress>();
            return uploadProgress;
        }

        private async Task<long> UploadBlocksAsync(HttpClient httpClient, Block[] blockToBeUploaded, int maxBlockSize,
            FileInfo uploadFile, Guid assetId, string authToken, string tenant, long bytesUploaded)
        {
            ...
            var uploadBlock = new ActionBlock<Block>(async block =>
                {
                    ...
                    try
                    {
                        ...
                        var bytesRead = await stream.ReadAsync(buffer, 0, (int)block.Size, CancellationToken.None).ConfigureAwait(false);
                        ...
                        await UploadBlockFromPayloadAsync(
                                httpClient,
                                authToken,
                                tenant,
                                block.Size,
                    ...
                    // from a midway-failed chunk upload. We'll reupload failed chunks later on so this is ok.
                    catch (Exception e)
                    {
            ...
            {
                await uploadBlock.SendAsync(block).ConfigureAwait(false);
            }
        ...
        private async Task UploadBlockFromPayloadAsync(HttpClient httpClient, string uploadToken, string tenant, long contentLength, Guid assetId, long blockId, byte[] payload)
        {
            ...
            {
                using (var cts = new CancellationTokenSource(_uploadConfig.HttpUploadTimeoutMs))
                {
        ...
        public async Task<UploadProgress> ContinueAssetAsync(HttpClient httpClient, string uploadToken, string tenant, Guid assetId)
        {
        ...
        private void ConfigureServicePointManager()
        {
            ...
            ServicePointManager.DefaultConnectionLimit = _uploadConfig.DefaultConnectionLimit == -1 ? 12 * Environment.ProcessorCount : _uploadConfig.DefaultConnectionLimit;

            // Disable the extra handshake on POST requests.
            ServicePointManager.Expect100Continue = _uploadConfig.Expect100Continue;

            // Turn off TCP small packet buffering (a.k.a. Nagle's algorithm)
            ServicePointManager.UseNagleAlgorithm = _uploadConfig.UseNagleAlgorithm;
        }
